 ...
 *
 * Modifications:
 * -Imported from com.google.samples.apps.iosched.util.SelectionBuilder
 * -Changed package name
 * -Changed log functions to key off of local DEBUG constant
 * -Replaced dependencies for Maps and Lists classes
 */
 ...
 * -Imported from AOSP frameworks/base/core/java/com/android/internal/bookName
 * -Changed package name
 */
...
public class SelectionBuilder {
    ...
    public Cursor query(SQLiteDatabase db, boolean distinct, String[] columns, String orderBy,
        ...
        if(DEBUG) Log.v(TAG, "query(columns=" + Arrays.toString(columns) + ", distinct=" + distinct + ") " + this);
        return db.query(distinct, mTable, columns, getSelection(), getSelectionArgs(), mGroupBy,
                mHaving, orderBy, limit);
    }

    public Cursor queryAll(SQLiteDatabase db, boolean distinct, String[] columns, String orderBy,
                        String limit) {
        assertTable();
        if (columns != null) mapColumns(columns);
        if(DEBUG) Log.v(TAG, "query(columns=" + Arrays.toString(columns) + ", distinct=" + distinct + ") " + this);
        return db.query(distinct, mTable, null, getSelection(), getSelectionArgs(), mGroupBy,
                mHaving, orderBy, limit);
    }
