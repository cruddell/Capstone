...
/**
 * Created by ChristopherRuddell, Museum of the Bible, on 7/12/16.
 */
public class RegistrationIntentService extends IntentService {
    ...
    protected void onHandleIntent(final Intent intent) {
        if (DEBUG_LOG) Log.d(TAG, "onHandleIntent");
        try {
            ...
            PrefUtils.setPrefSentTokenToServer(getApplicationContext(), true);
            // [END register_for_gcm]
        } catch (Exception e) {
            Log.d(TAG, "Failed to complete token refresh", e);
            // If an exception happens while fetching the new token or updating our registration data
            // on a third-party server, this ensures that we'll attempt the update at a later time.
            PrefUtils.setPrefSentTokenToServer(getApplicationContext(), false);
        }
    }
